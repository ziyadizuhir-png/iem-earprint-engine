...
class DynamicTargetLifecycleTests(unittest.TestCase):
    ...
    def _make_config(self, root):
        ...
        cfg = {
            ...
            "robust_mask": {
                "enabled": True,
                "retention_formula": (
                    "abs(centre)/(abs(centre)+total_uncertainty)"
                ),
                "zero_zero_rule": (
                    "Retention=0 and RawMask=0 when both "
                    "centre and total_uncertainty are exactly zero."
                ),
    ...
    def test_add_target_generates_outputs_and_removal_cleans_them(
        self
    ):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_curve(
                target_dir / "IEF2025.txt",
                freq,
                target_a,
            )
            self._write_curve(
                target_dir / "Target_B.txt",
                freq,
                target_b,
            )
