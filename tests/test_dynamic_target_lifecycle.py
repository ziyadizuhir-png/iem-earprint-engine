...
class DynamicTargetLifecycleTests(unittest.TestCase):
    ...
    The test runs the real engine main() against an isolated temporary project,
    so it verifies target discovery, robust-target output generation, and
    removal of stale target outputs without touching the repository dataset.
    """
    ...
    def _make_config(self, root):
        ...
        cfg = {
            ...
            "robust_mask": {
                ...
                "zero_zero_rule": "Retention=0 and RawMask=0 when both centre and total_uncertainty are exactly zero.",
    ...
    def test_add_target_generates_outputs_and_removal_cleans_them(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Two independent preferred responses with different shapes.
            ...
            self._write_curve(target_dir / "IEF2025.txt", freq, target_a)
            self._write_curve(target_dir / "Target_B.txt", freq, target_b)
            ...
            with patched:
                ...
                # Add a new target and rebuild.
                self._write_curve(target_dir / "Target_C.txt", freq, target_c)
                engine.main()
                ...
                # Remove the target and rebuild. Generated files for the
                # removed target must not survive as stale outputs.
