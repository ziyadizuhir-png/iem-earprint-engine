...
class DynamicIEMRegressionTests(unittest.TestCase):
    ...
    def test_adding_one_iem_changes_equal_vote_centre(self):
        ...
        a = self._median_aligned_curve(
            f,
            curve_a,
            reference,
            bands,
        )
        b = self._median_aligned_curve(
            f,
            curve_b,
            reference,
            bands,
        )
        c = self._median_aligned_curve(
            f,
            curve_c,
            reference,
            bands,
        ...
        centre_two = np.median(
            np.vstack([a, b]),
            axis=0,
        )
        centre_three = np.median(
            np.vstack([a, b, c]),
            axis=0,
        )
    ...
    def test_removing_added_iem_restores_previous_vote_centre(self):
        ...
        with_added = np.median(
            np.vstack(aligned[:3]),
            axis=0,
        )
        after_removal = np.median(
            np.vstack(aligned[:2]),
            axis=0,
        )
    ...
    def test_dynamic_discovery_count_changes_when_iem_is_added(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (preferred / "A.txt").write_text(
                "1 1\n",
                encoding="utf-8",
            )
            (preferred / "B.txt").write_text(
                "1 1\n",
                encoding="utf-8",
            )

            found_before = discover_txt(
                preferred,
                "*.txt",
                [],
            )
            ...
            (preferred / "C.txt").write_text(
                "1 1\n",
                encoding="utf-8",
            )

            found_after = discover_txt(
                preferred,
                "*.txt",
                [],
            )
