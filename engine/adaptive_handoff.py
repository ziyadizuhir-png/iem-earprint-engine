...
Nominally compatible case:
    f <= nominal_anchor -> BaseTarget
    f > nominal_anchor  -> Masked EarPrint

No correction is applied outside the defined handoff region.
"""
...
def log_curvature(freq, level):
    ...
    curvature[1:-1] = (slope[2:] - slope[:-2]) / (x[2:] - x[:-2])
    return curvature
...
def _curvature_stable(values, eps=1e-4):
    ...
    instability means repeated sign alternation or a degenerate field with
...
def _bridge_curvature_stable(curvature, eps=_EPS):
    """Validate cubic bridge curvature without banning one natural inflection.

    For a cubic Hermite bridge, second derivative is affine in log-frequency,
    so a valid smooth bridge may have zero or one curvature sign crossing.
    Repeated alternation indicates numerical/pathological behaviour.
    """
    if not np.all(np.isfinite(curvature)):
        return False
    active = curvature[np.abs(curvature) > eps]
    if active.size <= 1:
        return True
    signs = np.sign(active)
    changes = int(np.count_nonzero(signs[1:] != signs[:-1]))
...
def _c1_monotone_bridge(y0, y1, d0, d1, span_octaves):
    ...
    bridge_curvature = (6.0 * a * t + 2.0 * b) / (span_octaves ** 2)
    ...
    extrema = 0
    if active.size > 1:
        extrema = int(np.count_nonzero(np.sign(active[1:]) != np.sign(active[:-1])))

    curvature_stable = _bridge_curvature_stable(bridge_curvature)
    curvature_reversal = False
    active_curv = bridge_curvature[np.abs(bridge_curvature) > _EPS]
    if active_curv.size > 1:
        curvature_reversal = bool(
            np.any(np.sign(active_curv[1:]) != np.sign(active_curv[:-1]))
        )

    if (
        overshoot > _BRIDGE_TOL
        or undershoot > _BRIDGE_TOL
        or slope_reversal
        or extrema
        or not curvature_stable
    ):
        raise ValueError("Bridge failed monotonicity/curvature/overshoot constraints.")
    ...
    return {
        ...
        "bridge_min_curvature_db_per_octave2": float(np.min(bridge_curvature)),
        "bridge_max_curvature_db_per_octave2": float(np.max(bridge_curvature)),
...
def _local_metrics(freq, target, masked_target, nominal_hz, window_octaves):
    ...
    ts, ms = target_slope[idx], masked_slope[idx]
    tc, mc = target_curvature[idx], masked_curvature[idx]
    ...
    target_slope_scale = float(np.median(np.abs(ts)))
    masked_slope_scale = float(np.median(np.abs(ms)))
    slope_scale = max(target_slope_scale, masked_slope_scale, _EPS)
    normalized_slope_mismatch = local_slope_mismatch / slope_scale

    target_curvature_stable = _curvature_stable(tc)
    masked_curvature_stable = _curvature_stable(mc)
    slope_reversal_target = bool(
        ta.size > 1 and np.any(np.sign(ta[1:]) != np.sign(ta[:-1]))
    )
    slope_reversal_masked = bool(
        ma.size > 1 and np.any(np.sign(ma[1:]) != np.sign(ma[:-1]))
    )
    ...
    return {
        ...
        "normalized_slope_mismatch": float(normalized_slope_mismatch),
...
def nominal_seam_compatible(freq, target, masked_target, nominal_hz=1000.0, window_octaves=0.125):
    """Evaluate nominal seam compatibility using actual local geometry."""
    metrics = _local_metrics(freq, target, masked_target, nominal_hz, window_octaves)
    if metrics is None:
        return False, {"reason": "insufficient_local_samples_or_slope", "curvature_stable": False}
    ...
    if active.size > 1:
        artificial_extrema = int(np.count_nonzero(np.sign(active[1:]) != np.sign(active[:-1])))

    direction_ok = metrics["target_slope_direction"] == metrics["masked_slope_direction"]
    reversal_ok = not (metrics["slope_reversal_target"] or metrics["slope_reversal_masked"])
    curvature_ok = metrics["target_curvature_stable"] and metrics["masked_curvature_stable"]
    nominal_target_slope = float(np.interp(math.log(nominal_hz), np.log(freq), log_slope(freq, target)))
    nominal_masked_slope = float(np.interp(math.log(nominal_hz), np.log(freq), log_slope(freq, masked_target)))
    slope_match_ok = _slope_match_ok(nominal_target_slope, nominal_masked_slope)

    shape_ok = direction_ok and reversal_ok and artificial_extrema == 0 and curvature_ok and slope_match_ok
    return shape_ok, {
        ...
        "nominal_slope_mismatch_db_per_octave": abs(nominal_masked_slope - nominal_target_slope),
...
def _candidate_destination_stable(freq, masked_slope, masked_curvature, end_index, stability_window_octaves):
    ...
    if abs(d1) <= _EPS:
        return None
    slope_ok = _stable_window(masked_slope[stable_idx], np.sign(d1))
    curvature_ok = _curvature_stable(masked_curvature[stable_idx])
    if not slope_ok:
        return None
    return {
        "indices": stable_idx,
...
def _candidate_score(d0, d1, secant, span_octaves):
    """Deterministic geometric score; lower is better.

    Primary term: normalized endpoint-slope mismatch to the transition secant.
    Secondary term: transition width, so equally shaped candidates prefer the
    shorter handoff. No arbitrary weighted sum is used.
    """
    scale = max(abs(float(secant)), _EPS)
    normalized_endpoint_mismatch = max(
        abs(float(d0) - float(secant)),
        abs(float(d1) - float(secant)),
    ) / scale
    return (float(normalized_endpoint_mismatch), float(span_octaves))
...
def adaptive_masked_handoff(
    ...
    """Find earliest valid H, then best validated E for that H."""
    ...
    nominal_ok, nominal_diag = nominal_seam_compatible(freq, target, masked_target, nominal_hz)

    if nominal_ok:
        # Locked boundary rule: BaseTarget is preserved exactly through the
        # nominal anchor. Only above the anchor does the existing Masked
        # EarPrint continue unchanged.
        out = masked_target.copy()
        out[freq <= nominal_hz] = target[freq <= nominal_hz]
        return out, {
            ...
            "endpoint_target_slope_db_per_octave": _interp_log(freq, target_slope, nominal_hz),
            "endpoint_masked_slope_db_per_octave": _interp_log(freq, masked_slope, nominal_hz),
            "transition_secant_db_per_octave": None,
            "curvature_stable": nominal_diag.get("curvature_stable", False),
            "continuity_pass": True,
            "monotonicity_pass": True,
            "extrema_pass": nominal_diag.get("artificial_extrema_count", 0) == 0,
            "overshoot_pass": True,
            "undershoot_pass": True,
            "oscillation_pass": True,
            "slope_reversal_pass": not nominal_diag.get("slope_reversal", False),
            "c1_slope_match": True,
    ...
    candidates = np.where((freq > nominal_hz) & (freq < domain_end_hz))[0]

    # H is selected lexicographically first: the earliest stable compatible
    # handoff point. We deliberately do not allow a later H to beat an earlier
    # valid H merely because its E score is better.
    for hi in candidates:
        ...
        local_h_idx = np.where((freq >= H) & (freq <= H * 2**min_transition_octaves))[0]
        if local_h_idx.size < 3:
            continue
        target_active = target_slope[local_h_idx]
        target_active = target_active[np.abs(target_active) > _EPS]
        if target_active.size < 3 or not _sign_stable(target_active, np.sign(d0)):
            continue
        ...
        for ei in range(hi + 1, len(freq)):
            ...
            destination = _candidate_destination_stable(
                freq, masked_slope, masked_curvature, ei, stability_window_octaves
            )
            if destination is None or not destination["curvature_stable"]:
                continue

            try:
                bridge = _c1_monotone_bridge(y0, y1, d0, d1, span)
            except ValueError:
                continue

            score = _candidate_score(d0, d1, secant, span)
            valid.append({
                "ei": ei,
                "E": E,
                "span": span,
        ...
        # Best E is deterministic: geometric endpoint-vs-secant mismatch,
        # then shortest transition, then lowest E as a final deterministic tie-break.
        valid.sort(key=lambda c: (c["score"][0], c["score"][1], c["E"]))
        chosen = valid[0]
        ei = chosen["ei"]
        E = chosen["E"]
        span = chosen["span"]
        ...
        out = target.copy()
        out[freq > H] = masked_target[freq > H]
        bridge_idx = np.arange(hi + 1, ei + 1)
        t = np.log2(freq[bridge_idx] / H) / span
        out[bridge_idx] = (
            bridge["a"] * t**3 + bridge["b"] * t**2 + bridge["c"] * t + chosen["y0"]
        )
        ...
        return out, {
            ...
            "endpoint_target_level_db": chosen["y0"],
            "endpoint_masked_level_db": chosen["y1"],
            "endpoint_target_slope_db_per_octave": chosen["d0"],
            "endpoint_masked_slope_db_per_octave": chosen["d1"],
            "transition_secant_db_per_octave": chosen["secant"],
            "endpoint_slope_mismatch_db_per_octave": abs(chosen["d1"] - chosen["d0"]),
            "endpoint_to_secant_slope_ratio": max(abs(chosen["d0"]) / max(abs(chosen["secant"]), _EPS), abs(chosen["d1"]) / max(abs(chosen["secant"]), _EPS)),
            "candidate_count": len(valid),
            "selected_candidate_rank": 1,
            "candidate_score_normalized_endpoint_mismatch": chosen["score"][0],
            "candidate_score_transition_width_octaves": chosen["score"][1],
            "curvature_stable": destination["curvature_stable"],
            "destination_slope_stable": destination["slope_stable"],
            "destination_curvature_stable": destination["curvature_stable"],
            "bridge_curvature_stable": bridge["bridge_curvature_stable"],
            "continuity_pass": True,
            "monotonicity_pass": True,
            "extrema_pass": bridge["artificial_extrema_count"] == 0,
            "overshoot_pass": bridge["overshoot_db"] <= _BRIDGE_TOL,
            "undershoot_pass": bridge["undershoot_db"] <= _BRIDGE_TOL,
