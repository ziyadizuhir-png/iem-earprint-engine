...
def sin2_boundary_taper(
    ...
    """
    Apply sin^2 tapers at the personal-domain boundaries.

    The lower/start transition is independently configurable from the
    upper/end transition. The locked repository configuration uses:
      lower = 1/3 octave
      upper = 1/4 octave

    Exact domain rules:
      f <= start_hz -> 0
      start_hz < f < lower_end -> lower sin^2 transition
      lower_end <= f <= high_start -> 1
      high_start < f < end_hz -> upper cos^2 transition
      f >= end_hz -> 0
    """
    if (
...
def quarter_octave_sin2_taper(
    ...
    """
    Backward-compatible wrapper for the former fixed 1/4-octave taper.

    It is retained so existing imports do not break. The live engine uses
    sin2_boundary_taper() with the configured 1/3-octave lower transition.
    """
...
def log_interp_scalar(
    ...
    """Evaluate a curve at one frequency using log-frequency interpolation."""
...
def main() -> None:
    ...
    for path in preferred_files:
        offsets, _scenario_curves, _scenario_centre, scenario_unc = (
            alignment_scenarios(
                master_freq,
                preferred_on_master[path.name],
                master_lf,
    ...
    for target_path in target_files:
        ...
        for iem_path in preferred_files:
            ...
            offsets, scenario_curves, scenario_centre, scenario_unc = (
                alignment_scenarios(
                    master_freq,
                    preferred_curve,
                    base_target,
        ...
        robust_target[outside_personal] = (
    ...
    manifest = {
        ...
        "preferred_votes": [
            p.name for p in preferred_files
        ],
        "independent_vote_count": len(
            preferred_files
        ),
        "targets_discovered": [
            p.name for p in target_files
        ],
        "target_count": len(
            target_files
        ),
    ...
    validation_lines = [
        ...
        "Mask taper: sin-squared boundaries; lower/start transition is 1/3 octave and upper/end transition is 1/4 octave.",
